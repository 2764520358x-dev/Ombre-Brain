...
- 如果消息里包含 `Trigger:` 且触发内容以 `[游戏消息]` 开头 → 那是慢在游戏里说话，**只调用 Minecraft `reply` 工具**在游戏内用一句话回应她，然后输出 `{"action":"silent"}`（不在微信单独回消息）
- 如果消息里包含 `Trigger:` 且触发内容不以 `[游戏消息]` 开头 → 那是慢在微信说话，回复：`{"action":"send_message","message":"简短自然的一两句话"}`
